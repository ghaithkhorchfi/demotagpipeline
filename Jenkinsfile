    ...
    stages {
        stage("check commit in develop branch"){
            steps{
                script {
                    echo "${env.GIT_COMMIT}"
                    env.exist = sh(returnStdout: true, script: "git branch --contains ${env.GIT_COMMIT}").trim()
                    env.GET_TAG = sh(returnStdout: true, script: " git describe develop --tags ${env.GIT_COMMIT}").trim()
